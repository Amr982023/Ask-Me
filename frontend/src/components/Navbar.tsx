    ...
    <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-950/80 backdrop-blur border-b border-slate-200 dark:border-slate-800">
      <div className="max-w-4xl mx-auto px-4 h-16 flex items-center gap-3 justify-between">
        ...
        <nav className="flex items-center gap-2 sm:gap-3 shrink-0">
          {user ? (
            <>
              <Link to="/dashboard" className="hidden sm:block text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-brand-700 dark:hover:text-brand-400 px-3 py-2">
                Dashboard
              </Link>
              <Link to={`/u/${user.username}`} className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-brand-700 dark:hover:text-brand-400 px-3 py-2">
                My Profile
              </Link>
              {user.role === "Admin" && (
                <Link to="/admin" className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-brand-700 dark:hover:text-brand-400 px-3 py-2">
                  Admin
                </Link>
              )}
              <ThemeToggle />
            ...
            <>
              <ThemeToggle />
              <Link to="/login" className="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-brand-700 dark:hover:text-brand-400 px-3 py-2">
                Log in
              </Link>
